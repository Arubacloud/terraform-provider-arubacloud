...
### TD-035 — ~~`WaitForResourceDeleted` Not Applied to 20 of 27 Resources~~ **RESOLVED in PR #69**

All 27 resources now call `WaitForResourceDeleted` after a successful DELETE. The original 7 were fixed in the PR #68 commit; the remaining 20 were fixed in a follow-up commit on the same branch (PR #69). Every resource's Delete handler now polls GET until 404 before returning, preventing async-deletion race conditions.
