...
// TestResourceDelete_Happy verifies that Delete() succeeds (no error diagnostic)
// when the API returns 204 for DELETE and 404 for the polling GET.
func TestResourceDelete_Happy(t *testing.T) {
...
// TestResourceDelete_API500 verifies that Delete() adds an error diagnostic
...
// exits quickly without making the test suite slow.
func TestResourceDelete_API500(t *testing.T) {
	// Speed up the retry loop so tests finish in milliseconds.
